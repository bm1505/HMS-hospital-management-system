...
{
    // Load a font definition file with an absolute path
    $absolutePath = $_SERVER['DOCUMENT_ROOT'] . '/HMS-hospital-management-system-/laboratory/fpdf/font/' . $path;

    // Check if the file exists
    if (!file_exists($absolutePath)) {
        $this->Error('Font definition file not found: ' . $absolutePath);
    }

    // Include the font file
    include($absolutePath);

    // Check if the 'name' variable is set after including the font file
    if (!isset($name)) {
        $this->Error('Could not include font definition file: ' . $absolutePath);
    }

    // Handle encoding and subsetting settings if available
    if (isset($enc)) {
        $enc = strtolower($enc);
    }
    if (!isset($subsetted)) {
