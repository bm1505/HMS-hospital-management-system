...
// Fetch the role of the logged-in user (assuming $id is set in session)
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['send_to_nurse'])) {
        ...
        if ($stmt->execute()) {
            // Assuming $nurseID is available from session or elsewhere
            $nurseID = isset($_SESSION['nurseID']) ? $_SESSION['nurseID'] : 0;
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                ...
                <p class="text-center text-muted">
                    <marquee behavior="slow" direction="left">Fill the form to register a new patient.</marquee>
                </p>
                <?= $message ?>
            ...
            <!-- Right Column: Patients Waiting for Treatment -->
            <div class="col-md-6">
                    ...
                    <tbody>
                        ...
                        if ($waitingPatients->num_rows > 0) {
                            while ($patient = $waitingPatients->fetch_assoc()) {
                                echo "<tr>";
                                echo "<td>" . htmlspecialchars($patient['patientID']) . "</td>";
                            ...
                            echo "<tr><td colspan='6' class='text-center'>No patients waiting.</td></tr>";
                        }
                        ?>
                    </tbody>
                </table>
                <!-- Button to view all registered patients -->
                <div class="text-center mt-4">
                    <a href="display_patients.php" class="btn btn-success btn-lg">
                        <i class="fas fa-eye"></i> View Registered Patients
                    </a>
                </div>
            </div>
        </div>
